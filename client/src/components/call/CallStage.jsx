            ...
            <div className="flex-1 flex flex-col gap-3 min-h-0">
              <div className="flex-1 rounded-2xl overflow-hidden bg-black border border-white/10 relative shadow-2xl flex items-center justify-center">
                ...
                <div className="absolute top-3 right-3 flex items-center gap-2">
                  <button
                    onClick={() => setIsScreenMaximized(prev => !prev)}
                    ...
                    title={isScreenMaximized ? 'Restaurar visualização' : 'Ecrã em tela cheia'}
                  >
                    {isScreenMaximized ? <Minimize2 className="w-3.5 h-3.5" /> : <Maximize2 className="w-3.5 h-3.5" />}
                    <span>{isScreenMaximized ? 'Restaurar' : 'Tela Cheia'}</span>
                  </button>
                  <button
              ...
              {/* Participants Strip Below Screen Share (hidden in fullscreen/theater mode) */}
              {!isScreenMaximized && (
                <div className="h-16 sm:h-20 flex items-center gap-2.5 overflow-x-auto py-1 shrink-0">
                  {participants.map(p => {
                    ...
                    return hasCamera ? (
                      <div
                        key={p.socketId || p.id}
                        className="h-full aspect-video rounded-xl bg-zinc-900 border border-white/10 overflow-hidden relative shrink-0 flex items-center justify-center"
                      >
                        <VideoPlayer stream={camStream} isMirrored={isSelf} />
                        <div className="absolute bottom-1 left-1 right-1 flex items-center justify-between text-[10px] font-semibold bg-black/70 backdrop-blur-xs px-1.5 py-0.5 rounded-md text-white pointer-events-none">
                      ...
                      <div
                        key={p.socketId || p.id}
                        className="h-12 px-3 rounded-full bg-white/5 hover:bg-white/10 border border-white/5 flex items-center gap-2.5 shrink-0 transition-colors"
                      >
                        <Avatar
                          src={p.avatarUrl}
                          name={p.name}
                        ...
                        <div className="flex flex-col min-w-0">
                          <div className="flex items-center gap-1">
                            <span className="text-xs font-medium text-slate-200 max-w-[100px] truncate">{p.name}</span>
                            {p.isSpeaking && (
