...
import { Hash, Video, Play, CheckCircle2, HelpCircle, FileCode2, Plus, Settings, Sparkles } from 'lucide-react';
export const CourseServerSidebar = ({
  ...
  return <aside className="w-72 bg-white border-r border-slate-200 flex flex-col justify-between shrink-0 select-none z-20 shadow-xs">
      <div className="flex flex-col min-h-0 overflow-y-auto custom-scrollbar">
        ...
        <div className="h-16 px-4 border-b border-slate-200 bg-white flex items-center justify-between shrink-0">
          ...
          {isCreatorOrAdmin && <button onClick={() => onOpenCreateChannel('text')} className="p-2 rounded-xl hover:bg-slate-100 text-slate-500 hover:text-slate-900 transition-colors cursor-pointer" title="Criar novo canal no espaço" aria-label="Criar canal">
        ...
        <div className="p-3 flex flex-col gap-5">
          ...
          <div>
            ...
            <div className="flex flex-col gap-1">
              ...
              return <button key={room} onClick={() => onSelectLiveRoom(room)} className={`w-full flex items-center justify-between px-3 py-2 rounded-xl text-sm text-left transition-all cursor-pointer ${isActive ? 'bg-slate-900 text-white font-medium shadow-xs' : 'text-slate-700 hover:text-slate-950 hover:bg-slate-100 font-normal'}`}>
          ...
          <div>
            ...
            <div className="flex flex-col gap-3">
              {modules.map((mod, mIdx) => <div key={mod.id} className="flex flex-col gap-1">
                ...
                return <button key={les.id} onClick={() => onSelectLesson(les)} className={`w-full flex items-center justify-between px-3 py-2 rounded-xl text-left text-sm transition-all cursor-pointer ${isSelected ? 'bg-slate-900 text-white font-medium shadow-xs' : 'text-slate-700 hover:text-slate-950 hover:bg-slate-100 font-normal'}`}>
                        <div className="flex items-center gap-2.5 min-w-0">
                          {isCompleted ? <CheckCircle2 className={`w-4 h-4 shrink-0 ${isSelected ? 'text-emerald-400' : 'text-emerald-600'}`} /> : <Play className={`w-4 h-4 shrink-0 ${isSelected ? 'text-slate-300' : 'text-slate-400'}`} />}
