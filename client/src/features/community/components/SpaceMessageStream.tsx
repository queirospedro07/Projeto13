...
interface SpaceMessageStreamProps {
  currentChannel: any;
  messages: Message[];
  inputContent: string;
...
export const SpaceMessageStream: React.FC<SpaceMessageStreamProps> = ({
  currentChannel,
  messages,
  inputContent,
  onInputChange,
    ...
    <div className="flex-1 flex flex-col min-w-0 bg-white">
      ...
      <div className="h-16 px-6 border-b border-slate-200 bg-white flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <span className="font-bold text-slate-900 text-base truncate">{currentChannel?.name || 'geral'}</span>
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div className="relative hidden md:block">
            ...
            <input
              ...
              className="bg-slate-50 border border-slate-200 text-slate-900 rounded-xl pl-9 pr-3 py-1.5 text-xs focus:outline-none focus:border-blue-500 w-36 focus:w-52 transition-all"
            />
          </div>
          ...
          <button
            ...
            className={`p-2 rounded-xl border transition-colors cursor-pointer ${
              pinnedOnly ? 'bg-amber-50 text-amber-600 border-amber-300' : 'text-slate-500 hover:text-slate-800 border-slate-200'
            }`}
            title="Filtrar mensagens fixadas"
          >
          ...
          <button
            ...
            className={`p-2 rounded-xl border transition-colors cursor-pointer ${
              showMembers ? 'bg-blue-50 text-blue-600 border-blue-200' : 'text-slate-500 hover:text-slate-800 border-slate-200'
            }`}
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-4">
        {messages.length === 0 ? (
          ...
          messages.map((m) => (
            <div
              key={m.id}
              className={`group relative flex gap-3.5 p-3 rounded-2xl hover:bg-slate-50 transition-colors ${
                m.isPinned ? 'bg-amber-50/60 border border-amber-200/60' : ''
              }`}
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <span className="font-bold text-sm text-slate-900">{m.sender?.name || 'Membro'}</span>
                  {m.sender?.role === 'CREATOR' && (
                  ...
                  {m.isPinned && (
                    <span className="text-xs font-bold text-amber-600 flex items-center gap-1 ml-auto">
                  ...
                  <div className="flex items-center gap-1.5 mt-2 flex-wrap">
                    {m.reactions.map((r, rIdx) => (
                      <button
                        ...
                        className="px-2 py-0.5 rounded-lg bg-slate-100 hover:bg-blue-50 border border-slate-200 text-xs flex items-center gap-1 cursor-pointer transition-colors"
                      >
                        <span>{r.emoji}</span>
              ...
              <div className="absolute right-4 top-2 hidden group-hover:flex items-center gap-1 bg-white/95 backdrop-blur-md border border-slate-200/80 shadow-xs rounded-lg p-0.5">
                <button
                  onClick={() => onToggleReaction(m.id, 'gosto')}
                  className="p-1.5 hover:bg-slate-100 text-slate-500 hover:text-slate-900 rounded-md text-xs cursor-pointer transition-colors"
                  title="Gosto"
                  aria-label="Gosto"
                >
                ...
                {isOwnerOrAdmin && (
                  <button
                    ...
                    className={`p-1.5 hover:bg-slate-100 rounded-lg text-xs cursor-pointer ${
                      m.isPinned ? 'text-amber-600' : 'text-slate-500 hover:text-slate-900'
                    }`}
      ...
      {/* Replying banner */}
      {replyingTo && (
        <div className="px-6 py-2 bg-blue-50/80 border-t border-blue-100 flex items-center justify-between text-xs text-blue-900">
      ...
      {typingUsers.length > 0 && (
        <div className="px-6 py-1 text-xs text-slate-400 italic">
      ...
      <div className="p-4 border-t border-slate-200 bg-white">
        <form onSubmit={onSendMessage} className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-2xl px-4 py-2 focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-100 transition-all">
          <input
            type="text"
            placeholder={`Enviar mensagem em #${currentChannel?.name || 'geral'}...`}
            value={inputContent}
