...
export const SpaceMembersSidebar = ({
  space
}) => {
  ...
  })).filter(m => m.id !== space?.owner?.id);
  return <aside className="hidden lg:flex w-64 bg-slate-50 dark:bg-[#12141c] border-l border-slate-200 dark:border-[#222636] flex-col p-4 overflow-y-auto custom-scrollbar select-none shrink-0 transition-colors">
      ...
      <div>
        ...
        <div className="flex flex-col gap-1">
          {members.length > 0 ? members.map(m => <div key={m.id} className="flex items-center gap-2.5 p-2 rounded-xl hover:bg-white dark:hover:bg-[#171a24] transition-colors">
                <Avatar src={m.avatarUrl} name={m.name} size="sm" status="online" />
                <div className="min-w-0">
                  <p className="font-semibold text-slate-800 dark:text-zinc-200 text-sm truncate">
                    {m.name}
                  </p>
                  <p className="text-xs text-slate-400 dark:text-zinc-500">@{m.username}</p>
                </div>
              </div>) : <div className="p-3 rounded-xl bg-white dark:bg-[#171a24] border border-slate-200 dark:border-[#222636] text-center">
