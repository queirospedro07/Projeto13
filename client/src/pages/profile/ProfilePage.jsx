...
import { MapPin, Award, BookOpen, Flame, Users, Edit3, MessageSquare, UserPlus, UserCheck, Clock, Globe, Github, Instagram, Camera, ImagePlus, X, ExternalLink, Loader2, Eye, EyeOff } from 'lucide-react';
...
export const ProfilePage = () => {
  ...
  const [hideCompleted, setHideCompleted] = useState(() => {
    return localStorage.getItem('learnspace_hide_completed') === 'true';
  });
  const [hiddenCourseIds, setHiddenCourseIds] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem('learnspace_hidden_courses') || '[]');
    } catch {
      return [];
    }
  });

  const toggleHideCourse = (courseId) => {
    setHiddenCourseIds(prev => {
      const next = prev.includes(courseId) ? prev.filter(id => id !== courseId) : [...prev, courseId];
      localStorage.setItem('learnspace_hidden_courses', JSON.stringify(next));
      return next;
    });
  };

  const toggleHideAllCompleted = () => {
    setHideCompleted(prev => {
      const next = !prev;
      localStorage.setItem('learnspace_hide_completed', String(next));
      return next;
    });
  };
  ...
  return <div className="max-w-5xl mx-auto flex flex-col gap-8 animate-fade-in pb-16">
      ...
      <div className="relative rounded-3xl overflow-hidden border border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-sm">
        ...
        <div className="px-5 sm:px-10 pb-8 pt-0">
          
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-6">
            
            <div className="relative group -mt-14 sm:-mt-16 z-20 shrink-0 self-start">
            ...
            <div className="flex flex-wrap items-center gap-2.5 z-20 relative pt-1 sm:pt-0">
              {isMe ? <Button onClick={openEditModal} variant="secondary" size="md" leftIcon={<Edit3 className="w-4 h-4" />} className="font-bold cursor-pointer shadow-xs">
                  Editar Perfil
                </Button> : <>
      ...
      <div className="flex items-center gap-1.5 border-b border-slate-200 dark:border-zinc-800 pb-0 overflow-x-auto no-scrollbar">
        {[{
        id: 'courses',
        icon: BookOpen,
        label: 'Cursos'
      ...
      {activeTab === 'courses' && (() => {
        const visibleProfileCourses = myCourses.filter(e => {
          const isFinished = (e.progressPercent || 0) >= 100;
          if (isFinished && (hideCompleted || hiddenCourseIds.includes(e.course?.id))) {
            return false;
          }
          return true;
        });
        ...
        return (
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
              ...
              {myCourses.some(e => (e.progressPercent || 0) >= 100) && (
                <button
                  onClick={toggleHideAllCompleted}
                ...
                >
                  ...
                  <span>{hideCompleted ? 'Concluídos Ocultados' : 'Ocultar Concluídos'}</span>
                </button>
              )}
            </div>

            {visibleProfileCourses.length === 0 ? (
              <Card className="p-10 text-center bg-slate-50 dark:bg-zinc-800/50 border-slate-200 dark:border-zinc-700 rounded-3xl">
                ...
                <p className="text-sm font-bold text-slate-700 dark:text-zinc-300 mb-1">
                  {myCourses.length > 0 ? 'Os seus cursos concluídos estão ocultados.' : 'Nenhum curso ainda.'}
                </p>
                {myCourses.length > 0 ? (
                  <button
                    onClick={toggleHideAllCompleted}
                  ...
                  >
                    Voltar a exibir todos os cursos
                  </button>
                ) : (
                  <Link to="/explore">
                    ...
                    <Card key={e.id} hover className="p-4 flex items-center justify-between border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 shadow-xs rounded-2xl gap-4">
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="flex items-center justify-between mt-1 text-xs text-slate-500 dark:text-zinc-400">
                            <span>{Math.round(e.progressPercent || 0)}% completo</span>
                            {isFinished && isMe && (
                              <button
                                onClick={(ev) => {
                                  ev.preventDefault();
                                  ev.stopPropagation();
                                  toggleHideCourse(e.course?.id);
                                }}
