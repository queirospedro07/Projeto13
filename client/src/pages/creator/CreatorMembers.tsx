...
import { Users, Search, Shield, Award, Check, MessageSquare, ShieldAlert, Sparkles, CheckCircle2 } from 'lucide-react';
...
export const CreatorMembers: React.FC = () => {
  ...
  const [courseRoles, setCourseRoles] = useState<any[]>([]);
  ...
  const handleOpenRoleModal = async (member: any) => {
  ...
  const handleSaveRole = async () => {
    ...
    setSavingRole(true);
    try {
      await api.assignCourseMemberRole(
        roleModalMember.course.id,
        roleModalMember.user.id,
        selectedRoleId
      );
    ...
    <div className="max-w-6xl mx-auto flex flex-col gap-10 animate-fade-in pb-16">
      ...
      <div className="pb-6 border-b border-slate-200 dark:border-[#222636] flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div>
          ...
          <h1 className="text-3xl sm:text-4xl font-extrabold text-slate-900 dark:text-white tracking-tight">Gestão de Membros & Permissões</h1>
        ...
        <div className="relative w-full sm:w-80">
          ...
          <input
            ...
            className="w-full bg-white dark:bg-[#151720] border border-slate-200 dark:border-[#222636] focus:border-blue-500 rounded-2xl pl-10 pr-4 py-2.5 text-sm text-slate-900 dark:text-white placeholder:text-slate-400 focus:outline-none shadow-xs transition-all"
          />
        </div>
      </div>
      ...
      <Card className="p-0 overflow-hidden bg-white dark:bg-[#0c0d12] border-slate-200 dark:border-[#222636] shadow-sm">
        ...
        <div className="divide-y divide-slate-100 dark:divide-[#1e2130]">
          {loading ? (
            <div className="p-12 text-center text-sm font-medium text-slate-500 dark:text-slate-400">A carregar lista de membros...</div>
          ) : filteredMembers.length === 0 ? (
            <div className="p-12 text-center text-sm font-medium text-slate-500 dark:text-slate-400">Nenhum membro encontrado com este termo de pesquisa.</div>
          ) : (
            filteredMembers.map(m => (
              <div key={m.id} className="px-6 py-4 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-[#151720] transition-colors">
                <div className="flex items-center gap-3.5">
                  <Avatar src={m.user?.avatarUrl} name={m.user?.name} size="md" status="online" />
                  <div>
                    <span className="font-bold text-slate-900 dark:text-white text-sm">{m.user?.name}</span>
                ...
                <div className="flex items-center gap-8 text-sm">
                  ...
                  <span className="w-24 text-center text-blue-600 dark:text-blue-400 font-bold">Nvl {m.user?.level || 1} ({m.user?.xp || 0} XP)</span>
                  
                  {/* Role Assignment Column */}
                  <div className="w-28 flex justify-center">
                    <button
                      onClick={() => handleOpenRoleModal(m)}
                      className="px-2.5 py-1 rounded-lg text-xs font-bold border transition-all cursor-pointer flex items-center gap-1.5 shadow-2xs hover:scale-102 bg-blue-50 dark:bg-blue-950/40 text-blue-700 dark:text-blue-400 border-blue-200 dark:border-blue-900/50"
          ...
          <div className="bg-white dark:bg-[#12141c] rounded-3xl max-w-lg w-full border border-slate-200 dark:border-[#222636] shadow-2xl overflow-hidden my-auto flex flex-col">
            ...
            <div className="p-6 space-y-4 text-sm">
              <p className="text-xs text-slate-600 dark:text-slate-400">
                Selecione o cargo e conjunto de permissões que este membro terá no curso e nas salas de conferência:
              </p>

              {loadingRoles ? (
                <div className="p-6 text-center text-xs text-slate-400">A carregar cargos do curso...</div>
              ) : (
                <div className="space-y-2">
                  ...
                  <label
                    ...
                    className={`flex items-start gap-3 p-3.5 rounded-2xl border cursor-pointer transition-all ${
                      selectedRoleId === null
                        ...
                        : 'bg-white dark:bg-[#151720] border-slate-200 dark:border-[#222636] hover:bg-slate-50 dark:hover:bg-[#1a1d28]'
                    }`}
                  >
                    <input
                      type="radio"
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <span className="font-bold text-slate-900 dark:text-white text-sm">Aluno (Padrão)</span>
                        <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300">STUDENT</span>
                      </div>
                      <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                        Permissões normais de aluno: assiste aulas, pede autorização com mão levantada em chamadas com palco restrito.
                      </p>
                    </div>
                  </label>
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex flex-wrap gap-1.5 mt-2">
                          {role.canPostAnnouncements ? (
                            <span className="px-2 py-0.5 rounded-md bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-400 text-[10px] font-semibold">
                              ✓ Publicar comunicados
                            </span>
                          ) : null}
                          {role.canSpeakInStage ? (
                            <span className="px-2 py-0.5 rounded-md bg-indigo-50 dark:bg-indigo-950/40 text-indigo-700 dark:text-indigo-400 text-[10px] font-semibold">
                              ✓ Falar no palco direto
                            </span>
                          ) : null}
                          {role.canShareScreen ? (
                            <span className="px-2 py-0.5 rounded-md bg-purple-50 dark:bg-purple-950/40 text-purple-700 dark:text-purple-400 text-[10px] font-semibold">
                              ✓ Partilhar ecrã
                            </span>
                          ) : null}
                          {role.canModerateChat ? (
                            <span className="px-2 py-0.5 rounded-md bg-amber-50 dark:bg-amber-950/40 text-amber-700 dark:text-amber-400 text-[10px] font-semibold">
                              ✓ Moderar chat
                            </span>
                          ) : null}
                        </div>
                  ...
                  {courseRoles.length === 0 && (
                    <div className="p-4 rounded-xl bg-slate-50 dark:bg-[#151720] border border-slate-200 dark:border-[#222636] text-center text-xs text-slate-500">
                      Nenhum cargo personalizado configurado neste curso. Pode criar novos cargos na edição do curso.
                    </div>
                  )}
                </div>
              )}
            ...
            <div className="p-6 overflow-y-auto space-y-6 flex-1 text-slate-800 dark:text-slate-200 text-sm">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="p-4 rounded-2xl bg-slate-50 dark:bg-[#151720] border border-slate-200 dark:border-[#222636]">
                  <span className="text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">Progresso do Curso</span>
                ...
                <div className="p-4 rounded-2xl bg-slate-50 dark:bg-[#151720] border border-slate-200 dark:border-[#222636]">
                  <span className="text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">Aulas Concluídas</span>
                  <div className="text-2xl font-black text-blue-600 dark:text-blue-400 mt-1">14 / 17</div>
                  <span className="text-xs text-slate-500 dark:text-slate-400">Última aula há 2 horas</span>
                </div>
                <div className="p-4 rounded-2xl bg-slate-50 dark:bg-[#151720] border border-slate-200 dark:border-[#222636]">
                  <span className="text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">Média em Quizes</span>
                  <div className="text-2xl font-black text-purple-600 dark:text-purple-400 mt-1">94%</div>
                  <span className="text-xs text-slate-500 dark:text-slate-400">3 quizes realizados</span>
                </div>
              </div>

              {/* Lesson Completion Audit Log */}
              <div>
                <h4 className="font-extrabold text-slate-900 dark:text-white text-sm mb-3">Histórico de Aulas & Vídeos Assistidos</h4>
                <div className="rounded-2xl border border-slate-200 dark:border-[#222636] divide-y divide-slate-100 dark:divide-[#1e2130] overflow-hidden bg-slate-50/30 dark:bg-[#151720]/40">
                  <div className="p-3.5 flex items-center justify-between text-xs font-medium">
                    <span className="font-bold text-slate-800 dark:text-slate-200">1. Introdução à Arquitetura de Software</span>
                    <span className="text-emerald-600 dark:text-emerald-400 font-bold">100% Concluído • Há 3 dias</span>
                  </div>
                  <div className="p-3.5 flex items-center justify-between text-xs font-medium">
                    <span className="font-bold text-slate-800 dark:text-slate-200">2. Fundamentos de Performance em React</span>
                    <span className="text-emerald-600 dark:text-emerald-400 font-bold">100% Concluído • Há 2 dias</span>
                  </div>
                  <div className="p-3.5 flex items-center justify-between text-xs font-medium">
                    <span className="font-bold text-slate-800 dark:text-slate-200">3. Masterclass: Deploy & CI/CD Pipelines</span>
                    <span className="text-emerald-600 dark:text-emerald-400 font-bold">100% Concluído • Há 2 horas</span>
                  </div>
                </div>
              </div>
              ...
              <div className="border border-blue-200 dark:border-blue-900/50 bg-blue-50/30 dark:bg-blue-950/20 rounded-2xl p-5 space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  {essayGraded ? (
                    <span className="px-3 py-1 rounded-full bg-emerald-100 dark:bg-emerald-950/60 text-emerald-800 dark:text-emerald-400 font-bold text-xs">
                      Avaliado: {gradingScore}/100
                    </span>
                  ) : (
                    <span className="px-3 py-1 rounded-full bg-amber-100 dark:bg-amber-950/60 text-amber-800 dark:text-amber-400 font-bold text-xs">
                      Aguardar Correção
                    </span>
                  )}
                </div>
                ...
                <div className="p-3.5 rounded-xl bg-white dark:bg-[#151720] border border-slate-200 dark:border-[#222636]">
                  <span className="text-xs font-bold text-slate-400 dark:text-slate-500 block mb-1">Pergunta do Quiz:</span>
                ...
                <div className="p-3.5 rounded-xl bg-white dark:bg-[#151720] border border-slate-200 dark:border-[#222636]">
                  <span className="text-xs font-bold text-slate-400 dark:text-slate-500 block mb-1">Resposta Submetida pelo Aluno:</span>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 items-start pt-2">
                  <div className="sm:col-span-1">
                    <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">Nota (0 - 100)</label>
                    <input
                      type="number"
                      min={0}
                      max={100}
                      ...
                      className="w-full bg-white dark:bg-[#151720] border border-slate-200 dark:border-[#222636] focus:border-blue-500 rounded-xl px-3 py-2 text-sm font-bold text-slate-900 dark:text-white focus:outline-none"
                    />
                  </div>
                  <div className="sm:col-span-3">
                    <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">Comentário / Feedback do Instrutor</label>
                    <input
                      type="text"
                      value={gradingFeedback}
                      onChange={(e) => setGradingFeedback(e.target.value)}
                      placeholder="Escreva orientações para o aluno..."
                      className="w-full bg-white dark:bg-[#151720] border border-slate-200 dark:border-[#222636] focus:border-blue-500 rounded-xl px-3 py-2 text-xs text-slate-900 dark:text-white focus:outline-none"
                    />
                  </div>
                </div>
