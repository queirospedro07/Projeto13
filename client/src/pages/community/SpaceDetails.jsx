...
import { Users, Calendar, BookOpen, Download, Clock, Play } from 'lucide-react';
...
export const SpaceDetails = () => {
  ...
  const handleJoin = async () => {
    try {
      ...
      toast({
        title: 'Bem-vindo ao Espaço!',
        message: `Entrou com sucesso em ${space.name}.`
      });
      fetchSpace();
    } catch (err) {
      toast({
        title: 'Erro',
        message: 'Não foi possível entrar no espaço',
        type: 'error'
      });
    }
  };
  ...
  return <div className="max-w-6xl mx-auto flex flex-col gap-10 animate-fade-in pb-16">
      ...
      <div className="relative rounded-3xl overflow-hidden border border-slate-200 bg-white shadow-sm">
        ...
        <div className="p-8 sm:p-10 relative">
          <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between gap-6 -mt-20 sm:-mt-24 mb-6">
            <Avatar src={space.logoUrl} name={space.name} size="xl" className="ring-4 ring-white bg-white shadow-lg" />

            <div className="flex items-center gap-3">
              {space.isMember ? <Link to={`/learn/${space.slug || space.id}`}>
                ...
                </Link> : <Button onClick={handleJoin} variant="primary" size="lg" leftIcon={<Users className="w-5 h-5" />} className="font-bold shadow-md shadow-blue-500/20">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="bg-slate-50 border border-slate-200/80 rounded-2xl p-4 sm:p-5 flex flex-col items-start sm:items-end shrink-0 gap-1">
              ...
              <span className="text-[11px] text-slate-500 font-medium">
                Inclui aulas, canais e certificado
              </span>
            </div>
          </div>
      ...
      <div className="flex items-center gap-2 border-b border-slate-200 pb-3 overflow-x-auto">
        ...
        id: 'courses',
        label: `Aulas & Módulos (${space.courses?.length || 0})`
      }, {
        id: 'events',
        label: `Eventos & Lives (${space.events?.length || 0})`
      }, {
        id: 'members',
        label: `Membros (${space.membersCount || 0})`
      }, {
        id: 'resources',
        label: `Recursos (${space.resources?.length || 0})`
      }].map(tab => <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`px-5 py-2.5 rounded-xl font-semibold text-sm transition-all cursor-pointer shrink-0 ${activeTab === tab.id ? 'text-slate-900 bg-slate-100 border border-slate-300 shadow-xs' : 'text-slate-600 hover:text-slate-900 hover:bg-white border border-transparent'}`}>
            {tab.label}
          </button>)}
      </div>
      ...
      <div>
        {activeTab === 'home' && <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 flex flex-col gap-8">
              ...
              <Card className="p-7 bg-white border-slate-200 shadow-xs">
                ...
                <p className="text-base text-slate-600 leading-relaxed">
                  Participe nas conversas em{' '}
                  <Link to={`/community/${space.slug || space.id}/chat`} className="text-slate-900 font-bold hover:underline">
                    #geral
                  </Link>
                  , acompanhe as aulas integradas e participe nas mentorias ao vivo com partilha de
                  ecrã.
                </p>
              </Card>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h5 className="font-bold text-slate-900 text-base">
                          {space.courses[0].title}
                        </h5>
                        <p className="text-xs text-slate-500 font-medium mt-1">
                          {space.courses[0].difficulty} • {space.courses[0].durationHours}h de aulas
                        </p>
                      </div>
                    </div>
            ...
            <div className="flex flex-col gap-6">
              <Card className="p-6 border-slate-200 bg-white shadow-xs">
                ...
                <div className="flex flex-col gap-3">
                  {space.events?.length === 0 ? <p className="text-sm text-slate-500 font-medium py-4 text-center">
                      Sem eventos agendados de momento.
                    </p> : space.events?.map(ev => <div key={ev.id} className="p-4 rounded-2xl bg-slate-50 border border-slate-200 flex flex-col gap-1.5">
                        <span className="text-sm font-bold text-slate-900">{ev.title}</span>
                        <span className="text-xs text-slate-500 font-medium flex items-center gap-1.5">
                          <Clock className="w-3.5 h-3.5 text-slate-400" />
                          {new Date(ev.startDate).toLocaleDateString()} às{' '}
        ...
        {activeTab === 'courses' && <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {space.courses?.map(c => <Card key={c.id} hover className="p-5 flex flex-col justify-between border-slate-200 bg-white shadow-xs">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 font-medium mt-1">
                      {c.difficulty} • {c.durationHours}h de aulas
                    </p>
                  </div>
                </div>
        ...
        {activeTab === 'members' && <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {space.members?.map(m => <Card key={m.id} className="p-4 flex items-center justify-between border-slate-200 bg-white shadow-xs">
                <div className="flex items-center gap-3">
        ...
        {activeTab === 'events' && <div className="flex flex-col gap-4 max-w-3xl">
            {space.events?.map(ev => <Card key={ev.id} className="p-6 flex items-start justify-between border-slate-200 bg-white shadow-xs">
                <div>
        ...
        {activeTab === 'resources' && <div className="flex flex-col gap-3 max-w-3xl">
            {space.resources?.length === 0 ? <Card className="p-12 text-center text-slate-500 font-medium">
                Sem ficheiros adicionados a esta comunidade.
              </Card> : space.resources?.map(res => <div key={res.id} className="flex items-center justify-between p-5 rounded-2xl bg-white border border-slate-200 shadow-xs">
                  <div>
