...
export const CertificateView = () => {
  ...
  useEffect(() => {
    if (!id) return;
    api.verifyCertificate(id).then(data => setCert(data)).catch(() => {
      setCert(null);
  ...
  if (loading) return <div className="p-16 text-center text-slate-500 font-medium">
        A verificar credencial de certificado...
      </div>;
  if (!cert) {
    return <div className="max-w-xl mx-auto py-20 px-6 text-center animate-fade-in">
  ...
  return <div className="max-w-4xl mx-auto flex flex-col gap-8 py-8 animate-fade-in pb-16">
      ...
      <div className="border-4 border-slate-200 p-3 sm:p-4 rounded-3xl bg-slate-100 shadow-xl print:border-none print:p-0 print:shadow-none">
        <div className="relative border-2 border-slate-300 rounded-2xl p-10 sm:p-16 bg-white text-center select-none text-slate-900 overflow-hidden shadow-sm">
          ...
          <p className="text-sm text-slate-500 font-serif italic mb-3">Certifica-se que</p>
          ...
          <p className="text-sm text-slate-500 font-serif italic mb-4 max-w-xl mx-auto">
            concluiu com distinção todos os módulos teóricos, projetos práticos e avaliações do
            curso
          </p>
          ...
          <div className="mt-12 pt-4 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs text-slate-400 font-medium">
            <span>
              ID do Certificado: <strong className="text-slate-700">{cert.certificateId}</strong>
            </span>
