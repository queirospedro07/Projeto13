...
import { BookOpen, Play, CheckCircle2, ArrowRight, Eye, EyeOff } from 'lucide-react';
...
export const LibraryPage = () => {
  ...
  const filtered = enrollments.filter(e => {
    const isFinished = e.isCompleted || (e.progressPercent || 0) >= 100;
    const isManuallyHidden = hiddenCourseIds.includes(e.course?.id);
    if (isFinished && (hideCompleted || isManuallyHidden)) return false;
    if (filter === 'in-progress') return !e.isCompleted && !e.isCreator && (e.progressPercent || 0) < 100;
    if (filter === 'completed') return (e.isCompleted || (e.progressPercent || 0) >= 100) && !e.isCreator;
    ...
    <div className="max-w-7xl mx-auto flex flex-col gap-10 animate-fade-in pb-16">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 pb-6 border-b border-slate-200 dark:border-[#222]">
        ...
        <div className="flex items-center gap-1.5 bg-slate-100 dark:bg-[#121212] p-1.5 rounded-2xl border border-slate-200 dark:border-[#222] shadow-xs shrink-0 flex-wrap">
          ...
          {createdCount > 0 && (
            <button onClick={() => setFilter('created')} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer ${filter === 'created' ? 'bg-slate-950 dark:bg-white text-white dark:text-black shadow-xs' : 'text-slate-600 dark:text-zinc-400 hover:text-slate-900 dark:hover:text-white'}`}>
          ...
          <button
            ...
            className={`px-3 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5 border ${
              hideCompleted
                ...
                : 'bg-white dark:bg-[#181818] text-slate-700 dark:text-zinc-300 border-slate-200 dark:border-[#333] hover:border-slate-300'
            }`}
            title="Ocultar ou mostrar cursos já finalizados"
          >
            {hideCompleted ? <EyeOff className="w-3.5 h-3.5" /> : <Eye className="w-3.5 h-3.5" />}
            <span className="hidden sm:inline">{hideCompleted ? 'Concluídos Ocultos' : 'Ocultar Concluídos'}</span>
          </button>
        </div>
      </div>
        ...
        return <div key={enrollment.id} className="group rounded-3xl bg-white dark:bg-[#0e0e0e] border border-slate-200/90 dark:border-[#222] overflow-hidden shadow-xs hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between">
                ...
                <div className="relative aspect-video w-full bg-slate-900 overflow-hidden">
                  ...
                  <div className="absolute top-3.5 right-3.5 flex items-center gap-1.5">
                    {isFinished ? (
                      <>
                        <span className="px-3 py-1 rounded-xl bg-emerald-500/90 text-white font-extrabold text-xs shadow-md backdrop-blur-md flex items-center gap-1.5">
                        ...
                        <button
                          onClick={(e) => {
                            e.preventDefault();
                            e.stopPropagation();
                            toggleHideCourse(course.id);
                          }}
                          className="px-2 py-1 rounded-xl bg-black/75 hover:bg-black text-white text-[11px] font-bold shadow-md backdrop-blur-md flex items-center gap-1 cursor-pointer transition-colors"
                          title="Ocultar curso concluído"
                        >
                      ...
                      <span className="px-2.5 py-1 rounded-xl bg-blue-600/90 text-white font-extrabold text-xs shadow-md backdrop-blur-md">
                        {progress}%
                      </span>
                    )}
                  </div>
