...
import { LessonQATab, LessonQuestion } from '../../features/learn/components/LessonQATab';

export interface CourseChannel {
  id: string;
  name: string;
  type: 'text' | 'qa' | 'showcase' | 'voice' | 'live-room';
  topic: string;
...
export const CoursePlayer: React.FC = () => {
  ...
  const { user, triggerXpCelebration } = useAuth();
  ...
  // Selected View Mode
  const [activeView, setActiveView] = useState<'lesson' | 'channel' | 'live-room' | 'quiz' | 'documents'>('lesson');
  const [currentTextChannel, setCurrentTextChannel] = useState<CourseChannel | null>(null);
  const [customChannels, setCustomChannels] = useState<CourseChannel[]>([
    {
      id: 'ch-general',
      name: 'geral',
      type: 'text',
  ...
  // Lesson Tabs State
  const [activeLessonTab, setActiveLessonTab] = useState<'overview' | 'resources' | 'notes' | 'qa'>('overview');

  // Channel Chat Messages State
  ...
  const [editingChannel, setEditingChannel] = useState<CourseChannel | null>(null);
  const [newChannelName, setNewChannelName] = useState('');
  const [newChannelType, setNewChannelType] = useState<'text' | 'qa' | 'showcase' | 'voice' | 'live-room'>('text');
  const [newChannelTopic, setNewChannelTopic] = useState('');
  const [newChannelQuestion, setNewChannelQuestion] = useState('');
  const [isCreatingChannel, setIsCreatingChannel] = useState(false);

  // Rich Lesson Interactive States
  ...
  // Sync code editor when lesson changes
  useEffect(() => {
  ...
  const handleRunCode = () => {
    ...
    setTimeout(() => {
      ...
      toast({ title: 'Desafio Validado!', message: 'O seu código passou em todos os testes com sucesso.', type: 'success' });
    }, 900);
  };
  ...
  const fetchCourseData = async () => {
    ...
    try {
      ...
      if (!data) {
        data = {
          ...
          space: {
            ...
            channels: [
              { id: 'ch-general', name: 'geral', type: 'text', topic: 'Boas-vindas e conversas da turma.' },
              { id: 'ch-questions', name: 'duvidas-tecnicas', type: 'qa', topic: 'Espaço para dúvidas sobre as aulas.' },
              { id: 'ch-showcase', name: 'projetos-showcase', type: 'showcase', topic: 'Partilhe os seus projetos.' }
            ]
          }
        };
      }

      // Ensure modules with lessons exist
      let modulesList = data.modules || [];
      if (modulesList.length === 0 || !modulesList.some((m: any) => m.lessons && m.lessons.length > 0)) {
        modulesList = [
          {
            id: `mod-1-${data.id}`,
      ...
      data.userEnrollment?.lessonProgresses?.forEach((lp: any) => {
      ...
      if (allLessons.length > 0) {
        const firstUncompleted = allLessons.find((l: any) => !completed.has(l.id)) || allLessons[0];
        setCurrentLesson(firstUncompleted);
      }

      if (data.space?.channels && data.space.channels.length > 0) {
        const mapped = data.space.channels.map((c: any) => ({
          id: c.id,
          name: c.name,
          type: (c.type === 'voice' ? 'voice' : 'text') as any,
  ...
  // 2. Real-time Channel Messages Sync
  useEffect(() => {
    if (activeView === 'channel' && currentTextChannel) {
      joinChannel(currentTextChannel.id);

      api.getChannelMessages(currentTextChannel.id)
        .then(msgs => setChannelMessages(msgs || []))
        .catch(() => {
          setChannelMessages([
            {
              id: 'msg-welcome',
              ...
              sender: { name: course?.creator?.name || 'Instrutor', username: 'instrutor', role: 'CREATOR' }
            } as any
          ]);
        });
  ...
  const handleMarkComplete = async () => {
    ...
    try {
      ...
      toast({ title: 'Erro', message: 'Não foi possível marcar a lição como concluída', type: 'error' });
    }
  };
  ...
  const handleSaveChannelSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const newChannel: CourseChannel = {
        id: 'custom-' + Date.now(),
        name: formattedName,
        type: newChannelType,
  ...
  if (loading) {
    return (
      <div className="h-[calc(100vh-3.25rem)] flex items-center justify-center bg-[#090a0f] text-zinc-400 font-mono text-xs">
        <span className="w-2 h-2 rounded-full bg-emerald-500 animate-ping mr-2"></span>
        A carregar o ambiente do curso...
      </div>
    );
  }
  ...
  if (!course) {
    return (
      <div className="h-[calc(100vh-3.25rem)] flex items-center justify-center bg-[#090a0f] text-white">
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] overflow-hidden bg-slate-50 text-slate-900 selection:bg-blue-100">
      ...
      <CourseServerSidebar
        ...
        resourcesCount={course.resources?.length || 3}
        user={user}
        onSelectChannel={(ch) => {
        ...
        onSelectQuiz={() => setActiveView('quiz')}
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-slate-50 overflow-hidden">
        ...
        <header className="h-16 border-b border-slate-200 bg-white px-6 flex items-center justify-between shrink-0 shadow-xs">
            ...
            {activeView === 'channel' && (
              <div className="flex items-center gap-2.5">
                ...
                {currentTextChannel?.topic && (
                  <span className="text-sm text-slate-500 hidden md:inline truncate max-w-md">
                    — {currentTextChannel.topic}
                  </span>
                )}
              </div>
          ...
          <div className="flex items-center gap-3">
            ...
            {inLiveRoom && activeView !== 'live-room' && (
              <button
                onClick={() => setActiveView('live-room')}
                className="px-4 py-2 rounded-xl text-sm font-bold bg-blue-600 text-white hover:bg-blue-700 shadow-md flex items-center gap-2 cursor-pointer"
              >
        ...
        {/* VIEW 2: CHANNEL CHAT & DISCUSSIONS */}
        {activeView === 'channel' && currentTextChannel && (
          <CourseChannelChat
        ...
        {/* VIEW 3: LESSON CONTENT & MULTI-TYPE RUNNERS */}
        {activeView === 'lesson' && (
          <div className="flex-1 overflow-y-auto custom-scrollbar p-6 sm:p-8 max-w-5xl mx-auto w-full flex flex-col gap-8">
            ...
            {/* 2. INTERACTIVE CODING SANDBOX */}
            {currentLesson?.type === 'code' && (
              <div className="flex flex-col gap-4 bg-[#11131a] p-6 rounded-3xl border border-[#1e2230] text-white">
                <div className="flex items-center justify-between pb-3 border-b border-[#1e2230]">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <span className="text-[11px] text-emerald-400 font-mono">Sandbox Interativo • {currentLesson.codeChallenge?.language || 'TypeScript'}</span>
                    </div>
                  </div>
                ...
                <div className="p-4 rounded-2xl bg-[#181a24] border border-[#262a3b] text-xs text-zinc-300 leading-relaxed">
                  ...
                  <p>{currentLesson.codeChallenge?.instructions || currentLesson.content || 'Implemente a lógica solicitada e clique em "Executar e Testar".'}</p>
                </div>
                ...
                <div className="flex flex-col rounded-2xl overflow-hidden border border-[#262a3b] bg-[#0c0d12]">
                  <div className="flex items-center justify-between px-4 py-2.5 bg-[#141620] border-b border-[#262a3b] text-xs text-zinc-400">
                    ...
                    <button
                      ...
                      className="px-4 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs flex items-center gap-1.5 cursor-pointer shadow-md shadow-emerald-600/20 transition-all disabled:opacity-50"
                    >
                  ...
                  <textarea
                    ...
                    rows={8}
                    className="w-full bg-[#0c0d12] p-4 font-mono text-xs text-emerald-300 focus:outline-none resize-none"
                    spellCheck={false}
                  />

                  {/* Terminal Output */}
                  {codeOutput && (
                    <div className="p-4 border-t border-[#262a3b] bg-[#08090d] font-mono text-xs text-zinc-300 whitespace-pre-wrap animate-fade-in">
                      <span className="text-[10px] text-zinc-500 font-bold uppercase block mb-1">Terminal de Saída:</span>
              ...
              <div className="flex flex-col gap-4 bg-[#11131a] p-6 rounded-3xl border border-[#1e2230] text-white">
                <div className="flex items-center justify-between pb-3 border-b border-[#1e2230]">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <h2 className="text-base font-black text-white">{currentLesson.title}</h2>
                      <span className="text-[11px] text-zinc-400">Pacote de Assets, Guias e Ficheiros de Apoio</span>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                  {(currentLesson.resources || [
                    { id: 'r1', name: 'Starter-Kit-LearnSpace.zip', url: '#', size: '14.2 MB', type: 'zip' },
                    { id: 'r2', name: 'Guia-Referencia-Rapida.pdf', url: '#', size: '1.8 MB', type: 'pdf' },
                    { id: 'r3', name: 'Design-Tokens-UI.fig', url: '#', size: '6.4 MB', type: 'figma' }
                  ]).map((res: any) => (
                    <div
                      key={res.id}
                      className="p-4 rounded-2xl bg-[#181a24] border border-[#262a3b] flex items-center justify-between gap-3 hover:border-indigo-500 transition-colors group"
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <span className="text-[10px] text-zinc-400">{res.size} • Ficheiro Verificado</span>
                        </div>
                      </div>

                      <button
                        type="button"
                        onClick={() => toast({ title: 'Download Iniciado', message: `A descarregar ${res.name}`, type: 'success' })}
                        className="p-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white cursor-pointer shadow-md transition-all shrink-0"
            ...
            {/* 4. PRACTICAL PROJECT SUBMISSION */}
              ...
              <div className="flex flex-col gap-4 bg-[#11131a] p-6 rounded-3xl border border-[#1e2230] text-white">
                <div className="flex items-center justify-between pb-3 border-b border-[#1e2230]">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <h2 className="text-base font-black text-white">{currentLesson.title}</h2>
                      <span className="text-[11px] text-purple-400">Projeto Prático com Avaliação de Código</span>
                    </div>
                  </div>
                ...
                <div className="p-4 rounded-2xl bg-[#181a24] border border-[#262a3b] text-xs text-zinc-300 leading-relaxed flex flex-col gap-2">
                  ...
                  <p>{currentLesson.content || 'Desenvolva a solução completa e partilhe o link do seu repositório GitHub ou aplicação no showcase.'}</p>
                </div>
                ...
                <form
                  onSubmit={(e) => {
                    ...
                    toast({ title: 'Projeto Submetido', message: 'O seu projeto foi enviado para revisão dos mentores.', type: 'success' });
                  }}
                  className="flex flex-col gap-3 p-4 rounded-2xl bg-[#141620] border border-[#262a3b]"
                >
                  <label className="text-xs font-bold text-zinc-300">Link do Repositório GitHub / Demonstração Live</label>
                  <div className="flex gap-2">
                    <input
                      type="url"
                      placeholder="https://github.com/seu-utilizador/meu-projeto"
                      value={projectSubmissionUrl}
                      onChange={(e) => setProjectSubmissionUrl(e.target.value)}
                      className="flex-1 bg-[#181a24] border border-[#2b3044] rounded-xl px-4 py-2.5 text-xs text-white placeholder:text-zinc-500 focus:outline-none focus:border-indigo-500"
                    />
                    <button
                      type="submit"
                      className="px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs transition-all cursor-pointer shadow-md shadow-indigo-600/30 flex items-center gap-1.5"
                    >
            ...
            {/* 5. STANDARD VIDEO LESSON WITH CHAPTERS */}
            {currentLesson?.type === 'video' && (
              <div className="flex flex-col gap-4">
                <LessonVideoPlayer
                  videoUrl={currentLesson?.videoUrl}
                  title={currentLesson?.title || 'Aula em Vídeo'}
                  durationMin={currentLesson?.durationMin || 15}
            ...
            <div className="flex items-center justify-between">
              ...
              {nextLesson ? (
                <button
                  ...
                  className="px-6 py-3 rounded-2xl text-sm font-bold bg-blue-600 hover:bg-blue-700 text-white shadow-lg shadow-blue-500/20 flex items-center gap-2 cursor-pointer"
                >
                ...
                <button
                  onClick={handleMarkComplete}
                  className="px-6 py-3 rounded-2xl text-sm font-bold bg-emerald-600 hover:bg-emerald-700 text-white shadow-lg shadow-emerald-500/20 flex items-center gap-2 cursor-pointer"
                >
            ...
            <div>
              <div className="flex items-center gap-2 border-b border-slate-200 dark:border-[#222636] pb-3 text-sm font-bold text-slate-500 dark:text-slate-400 overflow-x-auto">
                {[
                  ...
                  { id: 'qa', label: `Dúvidas & Q&A (${questions.length})` },
                ].map(tab => (
                  <button
                    key={tab.id}
              ...
              <div className="pt-6">
                ...
                {activeLessonTab === 'resources' && (
                  <LessonResourcesTab resources={course.resources || []} />
                )}
        ...
        {/* VIEW 5: DOCUMENTS & REPOSITORY */}
        {activeView === 'documents' && (
          <CourseDocumentsView documents={course.resources} />
        )}

      </main>

      {/* 3. MODAL: CONFIGURAR / CRIAR CANAL NO CURSO (PARA O CRIADOR) */}
      {showCreateChannelModal && (
        <Modal
          isOpen={showCreateChannelModal}
          onClose={() => { setShowCreateChannelModal(false); setEditingChannel(null); }}
        ...
        >
          <form onSubmit={handleSaveChannelSubmit} className="flex flex-col gap-5">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {[
                  { name: 'anuncios', type: 'text', label: 'Anúncios', topic: 'Avisos importantes e comunicados da turma' },
                  { name: 'geral', type: 'text', label: 'Geral', topic: 'Conversas gerais e apresentações dos membros' },
                  { name: 'duvidas-aulas', type: 'qa', label: 'Dúvidas', topic: 'Espaço para colocar dúvidas de código e exercícios' },
                  { name: 'projetos-showcase', type: 'showcase', label: 'Projetos', topic: 'Partilha de repositórios GitHub e demos' },
                  { name: 'materiais-apoio', type: 'text', label: 'Materiais', topic: 'Links de apoio, PDFs e documentação recomendada' },
                  { name: 'mentoria-ao-vivo', type: 'voice', label: 'Mentoria Voz', topic: 'Sala de conferência ao vivo e mentoria' },
                ].map((tmpl) => (
                  <button
                    key={tmpl.name}
                    type="button"
                    onClick={() => {
                      ...
                      setNewChannelType(tmpl.type as any);
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                {[
                  { id: 'text', label: 'Texto & Chat', desc: 'Convívio geral' },
                  { id: 'qa', label: 'Dúvidas & Q&A', desc: 'Resolução técnica' },
                  { id: 'showcase', label: 'Showcase', desc: 'Projetos da turma' },
                  { id: 'voice', label: 'Sala de Voz', desc: 'Live e áudio' },
                ].map(t => (
                  <button
                    type="button"
                    key={t.id}
                    onClick={() => setNewChannelType(t.id as any)}
                  ...
                  >
                    <p className={`font-semibold ${newChannelType === t.id ? 'text-white' : 'text-slate-900'}`}>{t.label}</p>
                    <p className={`text-xs mt-1 ${newChannelType === t.id ? 'text-slate-300' : 'text-slate-500'}`}>{t.desc}</p>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  required
                  placeholder="ex: duvidas-modulo-2 ou projetos-finais"
                  value={newChannelName}
                  onChange={(e) => setNewChannelName(e.target.value)}
                  className="w-full bg-slate-50 border border-slate-300 focus:border-blue-600 focus:bg-white rounded-xl pl-9 pr-4 py-3 text-sm font-semibold text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-100 transition-all font-mono"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <input
                type="text"
                placeholder="ex: Canal dedicado a tirar dúvidas técnicas sobre a matéria do módulo"
                value={newChannelTopic}
                onChange={(e) => setNewChannelTopic(e.target.value)}
                className="w-full bg-slate-50 border border-slate-300 focus:border-blue-600 focus:bg-white rounded-xl px-4 py-3 text-sm font-medium text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-100 transition-all"
              />
            </div>
            ...
            <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-200">
              <button
                type="button"
                onClick={() => { setShowCreateChannelModal(false); setEditingChannel(null); }}
      ...
      {/* 4. MODAL: FAZER PERGUNTA / DÚVIDA */}
      {showAskModal && (
        <Modal
          isOpen={showAskModal}
          onClose={() => setShowAskModal(false)}
          title="Colocar Dúvida à Turma & Instrutor"
        >
          <div className="flex flex-col gap-4">
            <div>
              <label className="block text-sm font-bold text-slate-800 mb-1.5">
              ...
              <input
                type="text"
                placeholder="ex: Erro de tipagem no generic com useOptimistic"
                value={newQuestionTitle}
                onChange={(e) => setNewQuestionTitle(e.target.value)}
                className="w-full bg-slate-50 border border-slate-300 focus:border-blue-600 focus:bg-white rounded-xl px-4 py-3 text-sm font-semibold text-slate-900 placeholder:text-slate-400 focus:outline-none transition-all"
              />
            </div>
            ...
            <div>
              ...
              <textarea
                rows={5}
                placeholder="Descreva o que tentou implementar, código relevante e mensagem de erro..."
                value={newQuestionContent}
                onChange={(e) => setNewQuestionContent(e.target.value)}
                className="w-full bg-slate-50 border border-slate-300 focus:border-blue-600 focus:bg-white rounded-2xl p-4 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none transition-all font-mono"
              />
            </div>
              ...
              <button
                onClick={() => {
                  if (newQuestionTitle.trim()) {
                    setQuestions(prev => [
                      {
                        ...
                        upvotes: 1,
                        author: { name: user?.name || 'Você', avatarUrl: user?.avatarUrl }
                      },
                      ...prev
                    ]);
                    ...
                    soundEffects.playSuccess();
                    toast({ title: 'Pergunta Publicada!', message: 'A sua dúvida foi enviada para a turma e o instrutor.', type: 'success' });
                  }
                }}
