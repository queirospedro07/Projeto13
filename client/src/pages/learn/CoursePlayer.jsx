...
import { Play, Hash, Radio, HelpCircle, FileCode2, Check, Award, ChevronRight, ChevronLeft, Video, Code2, Download, FolderArchive, Lightbulb, CheckSquare, Square, Terminal, Rocket, Clock, CheckCircle2, Menu, BookOpen, Plus, Settings } from 'lucide-react';
...
export const CoursePlayer = () => {
  ...
  const isCreatorOrAdmin = user?.role === 'CREATOR' || user?.role === 'ADMIN' || user?.id === course?.creatorId || user?.id === course?.creator?.id;
  ...
  return <div className="flex h-[calc(100vh-4rem)] overflow-hidden bg-slate-50 text-slate-900 selection:bg-blue-100">
      ...
      <div className="hidden md:flex h-full shrink-0">
        <CourseServerSidebar courseTitle={course.title} courseId={course.id} isCreatorOrAdmin={isCreatorOrAdmin} customChannels={customChannels} currentTextChannel={currentTextChannel} activeView={activeView} inLiveRoom={inLiveRoom} liveRoomName={liveRoomName} modules={course.modules || []} currentLessonId={currentLesson?.id} completedLessonIds={completedLessonIds} resourcesCount={course.resources?.length || 0} user={user} onSelectChannel={ch => {
      ...
      {mobileSidebarOpen && (
        <div className="fixed inset-0 z-50 flex md:hidden">
          ...
          />
          <div className="relative flex flex-col w-72 max-w-[85vw] h-full bg-white z-10 shadow-2xl">
            <CourseServerSidebar
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-slate-50 overflow-hidden">
        ...
        <header className="h-16 border-b border-slate-200 bg-white px-4 sm:px-6 flex items-center justify-between shrink-0 shadow-xs">
          <div className="flex items-center gap-2 sm:gap-4 min-w-0">
            <button
              ...
              className="p-2 -ml-2 rounded-xl text-slate-600 hover:text-slate-900 hover:bg-slate-100 md:hidden transition-colors cursor-pointer shrink-0"
              title="Abrir menu do curso"
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            {isCreatorOrAdmin && (
              <Link
                to={`/creator/courses/${course.id}/edit?step=3`}
                className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs transition-colors cursor-pointer"
                title="Adicionar ou gerir lições e módulos"
              >
            ...
            {activeView === 'lesson' && currentLesson && <button onClick={handleMarkComplete} className={`px-3 sm:px-4 py-2 rounded-xl text-xs sm:text-sm font-bold flex items-center gap-1.5 sm:gap-2 transition-all cursor-pointer ${isCurrentCompleted ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-blue-600 text-white hover:bg-blue-700 shadow-md shadow-blue-500/20'}`}>
                {isCurrentCompleted ? <>
            ...
            {inLiveRoom && activeView !== 'live-room' && <button onClick={() => setActiveView('live-room')} className="px-3 sm:px-4 py-2 rounded-xl text-xs sm:text-sm font-bold bg-blue-600 text-white hover:bg-blue-700 shadow-md flex items-center gap-2 cursor-pointer">
        ...
        {activeView === 'lesson' && (!currentLesson ? (
            <div className="flex-1 flex flex-col items-center justify-center p-6 sm:p-8 text-center max-w-md mx-auto">
              ...
              <p className="text-sm text-slate-500 mb-6 leading-relaxed">
                {isCreatorOrAdmin
                  ? 'Este curso ainda não possui aulas adicionadas. Clique no botão abaixo para adicionar módulos e lições no editor do curso.'
                  : 'O instrutor ainda está a preparar os conteúdos deste curso. Fique atento às novidades!'}
              </p>
              {isCreatorOrAdmin && (
                <Link
                  to={`/creator/courses/${course.id}/edit?step=3`}
                  className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-sm shadow-md shadow-blue-500/20 transition-all cursor-pointer"
                >
                  <Plus className="w-4 h-4" />
                  <span>Adicionar Aulas & Módulos</span>
                </Link>
              )}
            </div>
          ) : (
            <div className="flex-1 overflow-y-auto custom-scrollbar p-4 sm:p-8 max-w-5xl mx-auto w-full flex flex-col gap-8">
