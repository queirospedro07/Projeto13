...
export const LibraryPage: React.FC = () => {
  ...
  const [filter, setFilter] = useState<'all' | 'in-progress' | 'completed' | 'created'>('all');
  ...
  return (
    <div className="max-w-7xl mx-auto flex flex-col gap-10 animate-fade-in pb-16">
        ...
        <div className="flex items-center gap-1.5 bg-slate-100 dark:bg-[#121212] p-1.5 rounded-2xl border border-slate-200 dark:border-[#222] shadow-xs shrink-0 flex-wrap">
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer ${
              filter === 'all' 
                ? 'bg-slate-950 dark:bg-white text-white dark:text-black shadow-xs' 
      ...
      {loading ? (
        ...
        <div className="py-20 text-center border-2 border-dashed border-slate-200 dark:border-[#222] rounded-3xl bg-white dark:bg-[#0a0a0a] max-w-xl mx-auto p-10 shadow-xs">
          ...
          <h3 className="text-xl font-extrabold text-slate-900 dark:text-white">Ainda não está inscrito em nenhum curso</h3>
          <p className="text-sm text-slate-500 dark:text-zinc-400 mt-2 mb-6">
            Explore o nosso catálogo e inscreva-se em cursos com aulas práticas, comunidade e salas de dúvidas.
          </p>
          <Link to="/community">
            <Button variant="primary" size="lg" className="font-bold">Explorar Catálogo de Cursos</Button>
          </Link>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {filtered.map(enrollment => {
            ...
            return (
              <div
                key={enrollment.id}
                className="group rounded-3xl bg-white dark:bg-[#0e0e0e] border border-slate-200/90 dark:border-[#222] overflow-hidden shadow-xs hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between"
              >
                {/* 16:9 Image Thumbnail Container with Overlays */}
                <div className="relative aspect-video w-full bg-slate-900 overflow-hidden">
                  ...
                  <div className="absolute top-3.5 left-3.5 flex items-center gap-2">
                    ...
                    {enrollment.isCreator && (
                      <span className="px-2.5 py-1 rounded-xl bg-blue-600/90 text-white backdrop-blur-md text-[11px] font-black border border-blue-400/30 shadow-sm uppercase tracking-wider">
                        Criador
                      </span>
                    )}
                  </div>
                  ...
                  {/* Instructor Pill at bottom of image */}
                  ...
                  <div>
                    ...
                    <div className="mt-4 pt-3 border-t border-slate-100 dark:border-[#1a1a1a]">
                      <div className="flex items-center justify-between text-xs font-bold mb-1.5">
                        <span className="text-slate-500 dark:text-zinc-400">Progresso do Curso</span>
