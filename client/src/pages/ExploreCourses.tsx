...
export const ExploreCourses: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto flex flex-col gap-8 animate-fade-in">
      ...
      {/* Header */}
      <div>
        <h1 className="text-3xl sm:text-5xl font-black text-slate-950 dark:text-white tracking-tight">Catálogo de Cursos</h1>
        <p className="text-base text-slate-600 dark:text-zinc-400 mt-2 max-w-3xl">
          Domine novas competências práticas através de cursos estruturados com canais de convívio, mentoria ao vivo e partilha de ecrã em tempo real.
        </p>
      </div>
      ...
      <div className="flex flex-col gap-5">
        <form onSubmit={handleSearchSubmit} className="flex gap-3">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Pesquise por título, tecnologia, criador ou palavra-chave..."
              value={search}
          ...
          <button
            type="submit"
            className="px-8 py-3.5 text-base font-bold text-white bg-blue-600 hover:bg-blue-700 shadow-md shadow-blue-500/20 rounded-2xl transition-all cursor-pointer"
          >
            Pesquisar
          </button>
        </form>
            ...
            <button
              ...
              className={`px-4 py-2.5 rounded-xl text-sm font-bold whitespace-nowrap transition-all cursor-pointer ${
                selectedCategory === cat.id
                  ...
                  : 'bg-white dark:bg-[#0a0a0a] border border-slate-200 dark:border-[#222222] text-slate-700 dark:text-zinc-300 hover:bg-slate-50 dark:hover:bg-[#141414] hover:text-slate-950 dark:hover:text-white'
              }`}
            >
              {cat.label}
            </button>
          ...
          <div className="flex items-center gap-4 flex-wrap">
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                className="bg-white dark:bg-[#0a0a0a] border border-slate-300 dark:border-[#222222] text-slate-800 dark:text-zinc-200 rounded-xl px-3 py-2 text-sm font-semibold focus:outline-none focus:border-blue-600 dark:focus:border-blue-500 shadow-2xs"
              >
      ...
      {loading ? (
        ...
        <div className="text-center py-20 border-2 border-dashed border-slate-200 dark:border-[#222222] rounded-3xl bg-white dark:bg-[#0a0a0a] p-8">
          <BookOpen className="w-12 h-12 text-slate-400 dark:text-zinc-600 mx-auto mb-3" />
          <h3 className="text-lg font-bold text-slate-800 dark:text-white">Nenhum curso encontrado</h3>
          <p className="text-sm text-slate-500 dark:text-zinc-400 mt-1">Experimente ajustar os filtros ou os termos de pesquisa.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {courses.map(course => (
            <div key={course.id} className="rounded-3xl bg-white dark:bg-[#0a0a0a] border border-slate-200 dark:border-[#222222] shadow-sm hover:shadow-2xl hover:border-blue-500/40 dark:hover:border-blue-500/40 transition-all duration-300 overflow-hidden flex flex-col justify-between group">
              {/* Thumbnail Container with Enhanced Overlays */}
              <div className="relative h-52 w-full bg-slate-900 overflow-hidden">
                <img
                  src={course.thumbnailUrl || 'https://images.unsplash.com/photo-1517694712202-14dd9538aa97?w=800&auto=format&fit=crop&q=80'}
                  alt={course.title}
                ...
                <div className="absolute top-3.5 inset-x-3.5 flex items-center justify-between pointer-events-none">
                  {/* Left: Category & Difficulty Badges */}
                  ...
                  {/* Right: Price / Acquired Badge */}
                  <div>
                    {course.isEnrolled ? (
                      <span className="px-3 py-1 rounded-xl bg-emerald-600/95 text-white backdrop-blur-md border border-emerald-400/40 text-xs font-black shadow-lg uppercase tracking-wider flex items-center gap-1.5">
                ...
                {/* Enrolled Progress Bar on Image Bottom */}
                {course.isEnrolled && (
                  <div className="absolute bottom-0 inset-x-0 h-1.5 bg-black/40">
                    <div
                      className="h-full bg-emerald-500 transition-all duration-300"
                      style={{ width: `${Math.min(100, Math.max(0, course.userProgress || 0))}%` }}
                    />
                  </div>
                )}
              </div>
